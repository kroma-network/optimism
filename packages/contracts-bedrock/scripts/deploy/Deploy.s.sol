...
contract Deploy is Deployer {
    ...
    function _run(bool _needsSuperchain) internal virtual {
        ...
        // Perform an upgrade for kroma proxy contracts only.
        upgradeKromaProxies();
        ...
        if (cfg.useFaultProofs()) {
            ...
            // TODO: 체크! initializeL2OutputOracle 함수 변경 필요
    ...
    // [Kroma: START]
    /// @notice Deploy kroma proxy contracts
    function deployKromaProxies(OPContractsManager.DeployInput calldata _input) public returns (DeployImplementationsOutput memory) {
        console.log("Deploying KromaProxies");
        uint256 l2ChainId = _input.l2ChainId;
        string memory saltMixer = _input.saltMixer;
        DeployImplementationsOutput memory output;
    ...
    function upgradeKromaProxies() public {
        console.log("Upgrade KromaProxies");
    ...
    function deployImplementations(bool _isInterop) public {
        ...
        // TODO : 생성자가 다양한 값들을 요구함.
        // 이를 위해서는 별도의 deployL2OutputOracle 함수를 만들어야 할 것 같음.(dii 필요)
        // deployL2OutputOracle();

        // Contracts in kroma require multiple proxy contract addresses when they are created,
        // so the addresses are obtained in advance through deployment to proxy contracts.
        OPContractsManager.DeployInput memory deployInput = getDeployInput();
        deployKromaProxies(deployInput);
        ...
        dii.set(dii.mipsVersion.selector, Config.useMultithreadedCannon() ? uint256(2) : uint256(1));
        ...
        dii.set(dii.kgh.selector, cfg.assetManagerBondAmount());
        dii.set(dii.securityCouncil.selector, cfg.assetManagerBondAmount());
        dii.set(dii.vault.selector, cfg.assetManagerBondAmount());
    ...
    function deployOpChain() public {
        ...
        OPContractsManager.DeployInput memory deployInput = getDeployInput();
        // TODO : opcm.deploy 에 추가한 L2OutputOracleProxy 도 추가되어야 함.
