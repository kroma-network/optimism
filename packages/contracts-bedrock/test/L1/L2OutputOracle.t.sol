...
// Target contract
import { KromaL2OutputOracle as L2OutputOracle } from "src/L1/KromaL2OutputOracle.sol";
...
contract L2OutputOracle_TestBase is CommonTest {
    ...
    function proposeAnotherOutput() public {
        ...
        emit OutputSubmitted(proposedOutput2, nextOutputIndex, nextBlockNumber, block.timestamp);

        vm.prank(trusted);
        l2OutputOracle.submitL2Output(proposedOutput2, nextBlockNumber, 0, 0);
    }
    ...
    function test_constructor_succeeds() external {
        ...
        assertEq(address(oracleImpl.COLOSSEUM()), address(0));
        assertEq(address(oracleImpl.colosseum()), address(0));
        assertEq(address(oracleImpl.VALIDATOR_MANAGER()), address(0));
        assertEq(address(oracleImpl.validatorManager()), address(0));
    ...
    function test_initialize_succeeds() external {
        ...
        assertEq(address(l2OutputOracle.COLOSSEUM()), address(colosseum));
        assertEq(address(l2OutputOracle.colosseum()), address(colosseum));
        assertEq(address(l2OutputOracle.VALIDATOR_MANAGER()), address(validatorManager));
        assertEq(address(l2OutputOracle.validatorManager()), address(validatorManager));
...
contract L2OutputOracle_getter_Test is L2OutputOracle_TestBase {
    ...
    function test_latestBlockNumber_succeeds() external {
        ...
        warpToSubmitTime();
        vm.prank(trusted);
        l2OutputOracle.submitL2Output(proposedOutput1, proposedNumber, 0, 0);
    ...
    function test_getL2Output_succeeds() external {
        ...
        warpToSubmitTime();
        vm.prank(trusted);
        l2OutputOracle.submitL2Output(proposedOutput1, nextBlockNumber, 0, 0);
    ...
    function test_getL2OutputIndexAfter_sameBlock_succeeds() external {
        ...
        warpToSubmitTime();
        vm.prank(trusted);
        l2OutputOracle.submitL2Output(output1, nextBlockNumber1, 0, 0);
        ...
        assertEq(
            ...
            keccak256(abi.encode(trusted, output1, block.timestamp, nextBlockNumber1))
        );
    }
    ...
    function test_getL2OutputIndexAfter_multipleOutputsExist_succeeds() external {
        ...
        assertEq(
            keccak256(abi.encode(l2OutputOracle.getL2Output(index1))),
            keccak256(
                abi.encode(trusted, output2, l2OutputOracle.computeL2Timestamp(nextBlockNumber2) + 2, nextBlockNumber2)
            )
        );
        ...
        assertEq(
            ...
            keccak256(
                abi.encode(trusted, output3, l2OutputOracle.computeL2Timestamp(nextBlockNumber3) + 2, nextBlockNumber3)
            )
        );
        ...
        assertEq(
            ...
            keccak256(
                abi.encode(trusted, output4, l2OutputOracle.computeL2Timestamp(nextBlockNumber4) + 2, nextBlockNumber4)
            )
        );
    }
    ...
    function test_getL2OutputIndexAfter_noOutputsExis_reverts() external {
        // Reset the l2Outputs array by setting its length to 0 in storage
        vm.store(address(l2OutputOracle), bytes32(uint256(3)), bytes32(uint256(0)));
...
contract L2OutputOracle_proposeL2Output_Test is L2OutputOracle_TestBase {
    ...
    function test_proposeWithBlockhashAndHeight_succeeds() external {
        ...
        warpToSubmitTime();
        vm.prank(trusted);
        l2OutputOracle.submitL2Output(nonZeroHash, nextBlockNumber, prevL1BlockHash, prevL1BlockNumber);
    }
    ...
    function test_proposeL2Output_notProposer_reverts() external {
        ...
        l2OutputOracle.submitL2Output(nonZeroHash, nextBlockNumber, 0, 0);
    }
    ...
    function test_proposeL2Output_emptyOutput_reverts() external {
        ...
        l2OutputOracle.submitL2Output(outputToPropose, nextBlockNumber, 0, 0);
    }
    ...
    function test_proposeL2Output_unexpectedBlockNumber_reverts() external {
        ...
        l2OutputOracle.submitL2Output(nonZeroHash, nextBlockNumber - 1, 0, 0);
    }
    ...
    function test_proposeL2Output_wrongFork_reverts() external {
        ...
        l2OutputOracle.submitL2Output(nonZeroHash, nextBlockNumber, bytes32(uint256(0x01)), block.number - 1);
    }
    ...
    function test_proposeL2Output_unmatchedBlockhash_reverts() external {
        ...
        vm.expectRevert("L2OutputOracle: block hash does not match the hash at the expected height");
        l2OutputOracle.submitL2Output(nonZeroHash, nextBlockNumber, l1BlockHash, l1BlockNumber - 1);
    }
}
...
contract L2OutputOracleUpgradeable_Test is L2OutputOracle_TestBase {
    ...
    function test_upgrading_succeeds() external {
        Proxy proxy = Proxy(kromaDeploy.mustGetAddress("KromaL2OutputOracleProxy"));
        ...
        proxy.upgradeToAndCall(address(nextImpl), abi.encodeCall(NextImpl.initialize, (3)));
