...
contract KromaGovernanceToken is KromaMintableERC20, ERC20Votes, Ownable2StepUpgradeable {
    /// @notice Address of the StandardBridge on this network (replaces immutable in KromaMintableERC20).
    address public bridge;

    /// @notice Address of the corresponding token on the remote chain (replaces immutable in KromaMintableERC20).
    address public remoteToken;

    /// @notice Constructs the KromaGovernanceToken contract.
    constructor() KromaMintableERC20(address(0), address(0), "", "") ERC20Permit("Kroma") {
    ...
    function initialize(address _owner, address _bridge, address _remoteToken) public reinitializer(2) {
        __Ownable2Step_init();
        bridge = _bridge;
    ...
    /// @notice Allows bridge or the owner to mint tokens.
    ...
    /// @notice Internal function that only allows the bridge to call
    function _checkBridge() internal view {
        require(msg.sender == bridge, "GovernanceToken: only bridge can call");
    }
